...
const CreateGalleryPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await toast.promise(
      axios.post(`${import.meta.env.VITE_API_URL}/api/gallery/create`, formData, {
      ...
      }),
      {
        ...
        success: (res) => {
          ...
          throw new Error(res.data.message || "Failed to create gallery.");
        },
  ...
  return (
    <div>
      <h1 className="font-bold text-white text-2xl">Create New Gallery</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-[#052B2D] mt-5 h rounded-md p-5">
        ...
        <div className="aspect-video md:w-1/4 mt-3 flex items-center justify-center  border-zinc-600 border-2 border-dashed p-2  rounded  overflow-hidden">
          {imagePreview ? <img src={imagePreview} alt="image preview" className={`w-full h-full  ${!imagePreview ? "brightness-100" : "object-cover"}`} /> : <p className="text-zinc-500">Gallery Image</p>}
        </div>
        ...
        <div className="mt-5 flex gap-2 flex-wrap">
          ...
          <Link to={loading ? "" : "/dashboard/galleries"}>
