...
const UpdateArticlePage = () => {
  ...
  if (error) {
    return (
      <div className="text-center mt-10 space-y-1">
        <h1 className="text-accent-700 font-semibold text-2xl">Article Not Found 🥲</h1>
        <p className="text-gray-400">
          The article you are looking for does not exist or could not be retrieved.{" "}
          <Link to={"/dashboard/articles"} className="underline  font-medium text-accent-600 italic">
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await toast.promise(
        axios.patch(`${import.meta.env.VITE_API_URL}/api/blog/update/${id}`, formData, {
  ...
  return (
    <div>
      <h1 className="font-bold text-white text-2xl">Edit Article</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-[#052B2D] mt-5 h rounded-md p-5">
        ...
        <div className="aspect-video md:w-1/4 mt-3 flex items-center justify-center  border-zinc-600 border-2 border-dashed p-2  rounded  overflow-hidden">
          {imagePreview ? <img src={imagePreview} alt="image preview" className={`w-full h-full  ${!imagePreview ? "brightness-100" : "object-cover"}`} /> : <p className="text-zinc-500">Article Image</p>}
        </div>
        ...
        <div className="mt-5 flex gap-2 flex-wrap">
          ...
          <Link to={submitLoading ? "" : "/dashboard/articles"}>
